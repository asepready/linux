...
openssl genrsa 2048 > /etc/ssl/certs/ca-key.pem

# Generate the CA certificate from the primary key
echo "Producing the primary certificate with the CA's key."
openssl req -new -x509 -nodes -days 730 -key /etc/ssl/certs/ca-key.pem -out /etc/ssl/certs/ca-cert.pem -subj "/C=US/ST=California/L=San Francisco/O=Adminbyaccident Ltd/CN=example.com/emailAddress=youremail@anymail.com"
...
openssl req -newkey rsa:2048 -days 730 -nodes -keyout /etc/ssl/certs/server-key.pem -out /etc/ssl/certs/server-req.pem -subj "/C=US/ST=State/L=City/O=Adminbyaccident Ltd/CN=example.com/emailAddress=youremail@anymail.com"

# 2.- Strip out the passphrase within the key
openssl rsa -in /etc/ssl/certs/server-key.pem -out /etc/ssl/certs/server-key.pem
...
openssl x509 -req -in /etc/ssl/certs/server-req.pem -days 730 -CA /etc/ssl/certs/ca-cert.pem -CAkey /etc/ssl/certs/ca-key.pem -set_serial 0100 -out /etc/ssl/certs/server-cert.pem
...
openssl req -newkey rsa:2048 -days 730 -nodes -keyout /etc/ssl/certs/client-key.pem -out /etc/ssl/certs/client-req.pem -subj "/C=US/ST=California/L=San Francisco/O=Adminbyaccident Ltd/CN=client.example.com/emailAddress=youremail@anymail.com"

# 2.- Strip out the passphrase within the key
openssl rsa -in /etc/ssl/certs/client-key.pem -out /etc/ssl/certs/client-key.pem
...
openssl x509 -req -in /etc/ssl/certs/client-req.pem -days 730 -CA /etc/ssl/certs/ca-cert.pem -CAkey /etc/ssl/certs/ca-key.pem -set_serial 0101 -out /etc/ssl/certs/client-cert.pem
...
openssl verify -CAfile /etc/ssl/certs/ca-cert.pem /etc/ssl/certs/server-cert.pem /etc/ssl/certs/client-cert.pem
